...
impl X86ExpressionCompiler {
    ...
    fn compile_simple(scope: &mut X86Store, inst_type: InstructionType, target: Value, source: Value) -> Vec<X86Instruction> {
    ...
    fn compile_div(scope: &mut X86Store, divider: Value, divided: Value, target_register: Register) -> Vec<X86Instruction> {
        let mut instructions = Vec::new();
    ...
    fn compile_single(scope: &mut X86Store, inst: InstructionType, source: Value) -> Vec<X86Instruction> {
        let mut instructions = Vec::new();
    ...
    pub fn compile_value(scope: &mut X86Store, value: Value) -> Vec<X86Instruction> {
        let mut instructions = Vec::new();
