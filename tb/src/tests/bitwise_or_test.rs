...
#[test]
fn basic_bitwise_or_test_1() {
    let mut main_func = FunctionType::main();
    let mut main_func_block = BlockType::default();

    main_func_block.add_assign("actual", ExpressionType::bitwise_or(Value::Number(101.into()), Value::Number(1.into())));
    ...
    get_exit_code(application_type, "bitwise_or_test1", 101);
}

#[test]
fn basic_bitwise_or_test_2() {
    let mut main_func = FunctionType::main();
    let mut main_func_block = BlockType::default();
    ...
    main_func_block.add_assign("actual", ExpressionType::bitwise_or(Value::Variable("test1".to_owned()), Value::Variable("test2".to_owned())));
    main_func_block.add_return_variable("actual");
    main_func.set_body(main_func_block);
    ...
    get_exit_code(application_type, "bitwise_or_test2", 11);
}