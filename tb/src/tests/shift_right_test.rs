...
#[test]
fn basic_shift_right_test_1() {
    let mut main_func = FunctionType::main();
    let mut main_func_block = BlockType::default();

    main_func_block.add_assign("actual", ExpressionType::shift_right(Value::Number(1.into()), Value::Number(5.into())));
    ...
    get_exit_code(application_type, "shift_right_test1", 2);
}

#[test]
fn basic_shift_right_test_2() {
    let mut main_func = FunctionType::main();
    let mut main_func_block = BlockType::default();
    ...
    main_func_block.add_assign("actual", ExpressionType::shift_right(Value::Variable("test1".to_owned()), Value::Variable("test2".to_owned())));
    main_func_block.add_return_variable("actual");
    main_func.set_body(main_func_block);
    ...
    get_exit_code(application_type, "shift_right_test2", 1);
}